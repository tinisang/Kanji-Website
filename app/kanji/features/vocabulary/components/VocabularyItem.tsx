...
import AddVocabularyToKanjiDialog from "./AddVocabularyToKanjiDialog";
...
export default function VocabularyItem({
  ...
  const [openAddKanji, setOpenAddKanji] = useState(false);

  useEffect(() => {
    async function fetchReview() {
      try {
        let item = await getReviewItemByTarget("kanji", vocabulary.id);

        if (!item) {
          item = await getReviewItemByTarget("vocabulary", vocabulary.id);
        }
  ...
  async function onChange<K extends keyof Vocabulary>(
    ...
    await updateVocabulary({ ...vocabulary, [key]: value });
  }
  ...
  function hasNote(note?: string | null) {
    return !!note && note.replace(/<[^>]+>/g, "").trim().length > 0;
  }
  ...
  function getReviewStateClass(state?: string) {
    switch (state) {
      ...
      case "relearning": return "bg-rose-100 text-rose-700";
      default: return "";
    }
  }

  const reviewState = review?.progress?.state;
  const isActiveReview = !!review && reviewState !== "review";
  const reviewStateLabel = isActiveReview
    ? getReviewStateLabel(reviewState)
    : null;
  const reviewStateClass = getReviewStateClass(reviewState);
  ...
  return (
    <>
      <div className={`w-full overflow-hidden rounded-xl border shadow-sm transition-all ${reviewClass}`}>
        <div className="flex flex-col gap-3 px-3 py-3 sm:flex-row sm:items-start sm:gap-4">
          <button
            type="button"
            onClick={() => setOpen(v => !v)}
            className="hidden shrink-0 rounded p-1 hover:bg-black/5 sm:mt-2 sm:block"
          >
          ...
          <div className="flex min-w-0 flex-1 flex-wrap items-center gap-3">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <EditableText
                ...
                placeholder="漢字"
                className="!w-full min-w-0 break-words whitespace-normal text-3xl font-bold leading-none sm:!w-[250px] sm:text-4xl"
                onSave={value => onChange("word", value)}
              />

              <button
                type="button"
                onClick={() => setOpenAddKanji(true)}
              ...
              {reviewStateLabel && (
                <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${reviewStateClass}`}>
                  {reviewStateLabel}
                </span>
              )}
            </div>
            ...
            <EditableText
              ...
              placeholder="かんじ"
              className="min-w-0 text-xl leading-none sm:text-2xl"
            ...
            <EditableText
              ...
              placeholder="Meaning"
              className="min-w-0 max-w-full text-base leading-normal sm:text-xl sm:leading-none"
              onSave={value => onChange("meaning", value)}
            />
          </div>

          <div className="flex shrink-0 items-center gap-1 border-t pt-2 sm:ml-auto sm:border-t-0 sm:pt-0">
            <button
              type="button"
              onClick={onMoveUp}
              disabled={index === 0}
      ...
      <AddVocabularyToKanjiDialog
        open={openAddKanji}
        onOpenChange={setOpenAddKanji}
        vocabulary={vocabulary}
      />
    </>
  );
